namespace HackF5.UnitySpy.Gui.Wpf.ViewModel
{
    ...
    public class MainViewModel : PropertyChangedBase
    {
        ...
        private async Task BuildImageAsync(ProcessViewModel process)
        {
            try
            {
                var windowsProcessFacade = new ProcessFacadeWindows(process.Process);
                ...
                this.Image = this.imageFactory(AssemblyImageFactory.Create(unityProcessFacade));
            }
            catch (Exception ex)
            {
        ...
        private async Task ExecuteRefreshProcesses()
        {
            ...
            this.SelectedProcess = this.Processes.FirstOrDefault(p => p.Name == "TheBazaar");
        }
    }
}