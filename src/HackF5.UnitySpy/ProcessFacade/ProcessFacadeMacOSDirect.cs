namespace HackF5.UnitySpy.ProcessFacade
{
    ...
    /// </summary>
    [PublicAPI]
    public class ProcessFacadeMacOSDirect : ProcessFacadeMacOS
    {
        public ProcessFacadeMacOSDirect(Process process)
            : base(process)
        {
        }
            ...
            int? size = default)
        {
            ...
            try
            {
                ...
                if (ProcessFacadeMacOSDirect.ReadProcessMemory(
                    this.Process.Id,
                    processAddress,
                    bufferPointer,
                    size ?? buffer.Length) != 0)
                {
                    ...
                    throw new InvalidOperationException($"Could not read memory at address {processAddress.ToString("X")}. Error code: {error}");
                }
            }
            finally
            {
